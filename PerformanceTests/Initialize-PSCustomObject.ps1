...
$Tests = @( foreach ( $loop in 1,3,8,13,21,100,1000,10000,100000){
    ...
    Test-Performance @splat -Name 'CastHash1'          -SB { [PSObject]@{Item = 1} }
    Test-Performance @splat -Name 'NewPSObj'           -SB { New-Object -TypeName PSObject -Property @{Item = 1} } 
    Test-Performance @splat -Name 'NewPSobjHASH'       -SB { [PSobject]::new(@{Item = 1} )  }
    ...
    Test-Performance @splat -Name 'NewPSobjCUSTOMHASH' -SB { [PSCUSTOMobject]::new(@{Item = 1} )  }
} )
